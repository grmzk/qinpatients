...
import Complaits from "./Complaints";
...
function FirstExamination({ id, patientInfo }: FirstExaminationProps) {
  ...
  const [complaintsState, setComplaintsState] = useState<TextareaExtendedState>();
  const [anamnesisMorbiState, setAnamnesisMorbiState] = useState<TextareaExtendedState>();

  const dispatch = useAppDispatch();

  useEffect(() => {
    console.log(complaintsState, anamnesisMorbiState);

    if (!complaintsState || !anamnesisMorbiState) {
      return;
    }
    dispatch(setEditorTabState({ id, state: { complaints: complaintsState, anamnesisMorbi: anamnesisMorbiState } }));
  }, [id, complaintsState, anamnesisMorbiState, dispatch]);
    ...
    <MainContentLayout>
      ...
      <div className={styles.mainArea}>
        ...
        <div className={styles.block}>
          <Complaits state={editorTabState?.complaints} setState={setComplaintsState} />
