      ...
      <div className={styles.main}>
        ...
        <div className={styles.optionsLayout}>
          <input
            type="checkbox"
            className={styles.option}
            id={"woundBandage" + uuid}
            checked={state.bandageChecked}
            onChange={() => setState({ ...state, bandageChecked: !state.bandageChecked })}
          />
          <label className={styles.optionLabel} htmlFor={"woundBandage" + uuid}>
            повязка
          </label>
          <input
            type="radio"
            className={styles.option}
            id={"woundDryBandage" + uuid}
            name={"woundBandage" + uuid}
            value={state.bandage}
            checked={state.bandage === "dry"}
            disabled={!state.bandageChecked}
            onChange={() => setState({ ...state, bandage: "dry" })}
          />
          <label className={styles.optionLabel} htmlFor={"woundDryBandage" + uuid}>
            сухая
          </label>
          <input
            type="radio"
            className={styles.option}
            id={"woundBloodBandage" + uuid}
            name={"woundBandage" + uuid}
            value={state.bandage}
            checked={state.bandage === "blood"}
            disabled={!state.bandageChecked}
            onChange={() => setState({ ...state, bandage: "blood" })}
          />
          <label className={styles.optionLabel} htmlFor={"woundBloodBandage" + uuid}>
            пропитана кровью
          </label>
        </div>
      </div>
